...
const lesson = { id: 'eco-2026-09-08', title: 'Aula', lessonDate: '2026-09-08' };
const photoDataUrl =
...
test('hidden Eco QR page points to the attendance flow', async ({ page }) => {
  ...
  await expect(page.getByText('Aula - 08/09/2026')).toBeVisible();
  await expect(page.getByRole('img', { name: /qr code/i })).toBeVisible();
  await expect(page.getByRole('link', { name: /abrir presença/i })).toHaveAttribute('href', /\/eco\/presenca\?data=2026-09-08/);
...
test('Eco attendance form sends name phone and selfie to backend', async ({ page }) => {
  ...
  await page.route('**/api/v1/eco/attendance', async (route) => {
    ...
    expect(body.lessonDate).toBe('2026-09-08');
  ...
  await page.setViewportSize({ width: 390, height: 844 });
  await page.goto('/eco/presenca?data=2026-09-08', { waitUntil: 'domcontentloaded' });
...
test('admin can open Eco lesson, inspect selfie and validate attendance', async ({ page }) => {
  ...
  await page.route('**/api/v1/admin/eco/lessons/eco-2026-09-08/attendances', async (route) => route.fulfill({ json: [attendance] }));
  await page.route('**/api/v1/admin/eco/lessons/eco-2026-09-08/attendances/att-01/validation', async (route) => {
    validationHit = true;
    const body = route.request().postDataJSON();
  ...
  await expect(page.getByRole('button', { name: /aula - 08\/09\/2026/i })).toBeVisible();
