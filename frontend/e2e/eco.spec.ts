...
const lesson = { id: 'eco-2026-09-01', title: 'Aula', lessonDate: '2026-09-01' };
const photoDataUrl =
...
test.beforeEach(async ({ page }) => {
  ...
  await page.route('**/api/v1/eco/students/lookup?**', async (route) => route.fulfill({ status: 204 }));
});
...
test('hidden Eco QR page points to the attendance flow', async ({ page }) => {
  ...
  await expect(page.getByText('Aula - 01/09/2026')).toBeVisible();
  await expect(page.getByRole('img', { name: /qr code/i })).toBeVisible();
  await expect(page.getByRole('link', { name: /abrir presença/i })).toHaveAttribute('href', /\/eco\/presenca\?data=2026-09-01/);
  await expectNoHorizontalOverflow(page);
});

test('Eco attendance form sends name phone and selfie to backend', async ({ page }) => {
  let posted = false;
  await page.route('**/api/v1/eco/students/lookup?**', async (route) =>
    route.fulfill({ json: { name: 'Aluno Eco', phone: '(81) 99949-9159' } })
  );
  await page.route('**/api/v1/eco/attendance', async (route) => {
    ...
    expect(body.lessonDate).toBe('2026-09-01');
  ...
  await page.setViewportSize({ width: 390, height: 844 });
  await page.goto('/eco/presenca?data=2026-09-01', { waitUntil: 'domcontentloaded' });
...
test('admin can open Eco lesson, inspect selfie and validate attendance', async ({ page }) => {
  ...
  await page.route('**/api/v1/admin/eco/lessons/eco-2026-09-01/attendances', async (route) => route.fulfill({ json: [attendance] }));
  await page.route('**/api/v1/admin/eco/lessons/eco-2026-09-01/attendances/att-01/validation', async (route) => {
    validationHit = true;
    const body = route.request().postDataJSON();
  ...
  await expect(page.getByRole('button', { name: /aula - 01\/09\/2026/i })).toBeVisible();
